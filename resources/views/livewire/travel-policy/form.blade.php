<div class="p-6 bg-white dark:bg-gray-800 shadow-md sm:rounded-lg">
    ...
    @if (session()->has('message') || session()->has('error'))
        <div id="toast-message" class="fixed bottom-4 right-4 flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert"
             x-data="{ show: true }"
             x-show="show"
             x-init="setTimeout(() => show = false, 3000)">
            <div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-lg {{ session()->has('error') ? 'text-red-500 bg-red-100 dark:bg-red-800 dark:text-red-200' : 'text-green-500 bg-green-100 dark:bg-green-800 dark:text-green-200' }}">
                @if(session()->has('error'))
                ...
                @endif
                <span class="sr-only">{{ session()->has('error') ? 'Error' : 'Success' }} icon</span>
            </div>
            <div class="ms-3 text-sm font-normal">{{ session()->get('error') ?? session()->get('message') }}</div>
        </div>
    @endif
</div> 