<div class="p-6 bg-white dark:bg-gray-800 shadow-md sm:rounded-lg">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Branches</h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Manage branch information and local government tax rates
            </p>
        </div>
        <div>
            <a 
                href="{{ route('maintenance.branches.create') }}"
                wire:navigate
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th wire:click="sortBy('lgt_rate')" scope="col" class="px-6 py-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600">
                        <div class="flex items-center">
                            LGT Rate
                            @if ($sortField === 'lgt_rate')
                                <svg class="w-3 h-3 ml-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                    @if ($sortDirection === 'asc')
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                    @else
            ...
            <tbody>
                @forelse ($branches as $branch)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4">
                            {{ number_format($branch->lgt_rate, 2) }}
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex justify-end gap-2">
                                <a 
                                    href="{{ route('maintenance.branches.show', $branch) }}"
                                    wire:navigate 
